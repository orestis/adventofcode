...
(def input (-> "2017/day9.txt" io/resource slurp))
...
(defn start-group [s groups] (parse s groups))
(defn end-group [s groups] (parse s groups))
(defn start-garbage [s groups] (parse s groups))
(defn end-garbage [s groups] (parse s groups))

(declare parse-in-garbage)
(declare parse-in-group)

(defn begin-group [[nest-level score garbage]] [(inc nest-level) (+ score nest-level) garbage])
(defn end-group [[nest-level score garbage]] [(dec nest-level) score garbage])
(defn count-garbage [[nest-level score garbage]] [nest-level score (inc garbage)])

(defn parse-in-group [s groups]
    (case (first s)
      \{ (recur (rest s) (begin-group groups))
      \} (recur (rest s) (end-group groups))
      \< (parse-in-garbage (rest s) groups)
      nil groups
      (recur (rest s) groups) ))

(defn parse-in-garbage [s groups]
    (case (first s)
      \! (recur (rest (rest s)) groups)
      \> (parse-in-group (rest s) groups)
      (recur (rest s) (count-garbage groups)) ))

(parse-in-group "{{{}}}" [1 0 0])
