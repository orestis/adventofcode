...
(defn generate-value [^long prev ^long factor]
...
(generate-value 65 A)
(generate-value 8921 B)

(count
(Integer/toString 65535 2))


(defn count-pairs [startA startB steps genA genB]
  (loop [prevA startA prevB startB c 0 n ^long steps]
    (let [vA (genA prevA A) vB (genB prevB B)]
      (if (= n 0) c
      (if (= (bit-and vA 65535) (bit-and vB 65535))
        (recur vA vB (inc c) (dec n))
        (recur vA vB c (dec n))
        )))))
...
(count-pairs 65 8921 40000000 generate-value generate-value)
;; => 588

;; Generator A starts with 722
;; Generator B starts with 354

(count-pairs 722 354 40000000 generate-value generate-value)
;; => 612
...
(defn generator [ multiple]
  (fn [prev factor]
    (loop [v (generate-value prev factor)]
      (if (= 0 (rem v multiple)) v (recur (generate-value v factor))))))
...
(def genA (generator 4))
(def genB (generator 8))
...
(count-pairs 65 8921 1056 genA genB)
;; => 1

(count-pairs 65 8921 5000000 genA genB)
;; => 309
...
(count-pairs 722 354 5000000 genA genB)
;; => 285
