...
(def input [63,144,180,149,1,255,167,84,125,65,188,0,2,254,229,24])
...
;; size (which starts at 0), and a sequence of lengths (your puzzle input).
...
(defn hash-knot [coll pos length]
  (loop [t (transient coll)
         ...
         indices (for [i (range pos (+ pos length))] (mod i (count coll)))]
    (let [el (first r) idx (first indices)]
      (if (nil? el) (persistent! t)
        (recur (assoc! t idx el) (rest r) (rest indices))))))


(with-test
  (defn hash-step [{:keys [list pos skip]} length]
    {:list (hash-knot list pos length)
     :pos (mod (+ pos skip length) (count list))
     :skip (inc skip)}
  )
...
(is (= {:list [2 1 0 3 4] :pos 3 :skip 1} (hash-step {:list [0 1 2 3 4] :pos 0 :skip 0} 3)))

;;     The second length, 4, selects a section which wraps: 2 1) 0 ([3] 4.
;;     The sublist 3 4 2 1 is reversed to form 1 2 4 3: 4 3) 0 ([1] 2.
;;     The current position moves forward by the length plus the skip size, a total of 5, causing it not to move because it wraps around: 4 3 0 [1] 2. The skip size increases to 2.

  (is (= {:list [4 3 0 1 2] :pos 3 :skip 2} (hash-step {:list [2 1 0 3 4] :pos 3 :skip 1} 4)))
;;     The third length, 1, selects a sublist of a single element, and so reversing it has no effect.
;;     The current position moves forward by the length (1) plus the skip size (2): 4 [3] 0 1 2. The skip size increases to 3.
  (is (= {:list [4 3 0 1 2] :pos 1 :skip 3} (hash-step {:list [4 3 0 1 2] :pos 3 :skip 2} 1)))

;;     The fourth length, 5, selects every element starting with the second: 4) ([3] 0 1 2. Reversing this sublist (3 0 1 2 4 into 4 2 1 0 3) produces: 3) ([4] 2 1 0.
;;     Finally, the current position moves forward by 8: 3 4 2 1 [0]. The skip size increases to 4.
  (is (= {:list [3 4 2 1 0] :pos 4 :skip 4} (hash-step {:list [4 3 0 1 2] :pos 1 :skip 3} 5)))
)
...
;; list?
(defn solve [lengths list-size]
  (let [s (reduce hash-step {:list (vec (range 0 list-size)) :pos 0 :skip 0} lengths)]
    (apply * (take 2 (:list s)))))
