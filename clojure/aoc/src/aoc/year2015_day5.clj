(ns aoc.year2015-day5
  (:require [clojure.java.io :as io])
  (:require [clojure.string :as str]))

(def input
  (-> "2015/day5.txt" io/resource slurp str/split-lines))
...
(defn vowel-count
  [s]
  (count (filter #(str/index-of "aeiou" %) s)))
...
(defn has-pair?
  ...
  (let [pairs (partition 2 1 s)]
    (some #(= (first %) (second %)) pairs)))

(has-pair "xx")
(has-pair "xy")
(has-pair "abcdde")

(defn contains-naughty?
  [s]
  (let [pred (partial str/includes? s)]
    (some pred ["ab" "cd" "pq" "xy"])))

(contains-naughty "aaaa")
(contains-naughty "aaba")

(defn is-nice?
  [s]
  (and (not (contains-naughty? s))
       (has-pair? s)
       (>= (vowel-count s) 3)))
