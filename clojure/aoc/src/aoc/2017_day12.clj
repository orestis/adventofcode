...
(def input
  (-> "2017/day12.txt" io/resource slurp str/split-lines))
...
(defn make-graph [input]
  (let [adjacency-map (into {} (map parse input))]
    (uber/graph adjacency-map)))

(def sample-graph
  (make-graph sample-input))

(uber-alg/scc sample-graph)
;; => [[1] [5 6 4 3 2 0]]
