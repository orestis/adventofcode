...
(defn run [states tape start-pos start-state steps]
  (loop [t (transient tape) ^long pos start-pos state start-state c 0]
    (if (= c steps) (reduce + (vals (persistent! t)))
        (let [[to-write ^long dir new-state] (get-in states [state (get t pos 0)])]
          (recur (assoc! t pos to-write) (+ pos dir) new-state (inc c))))))

(run sample-states {} 0 :a 6)
;; => 3
...
(run puzzle-states {} 0 :a 12134527);; => 5593
